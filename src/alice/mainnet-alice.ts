...
export class MainnetAlice {
  ...
  async createOrder(params: OrderParams): Promise<string> {
    ...
    // Build the extension data with factory address + extraDataArgs
    ...
    // Build maker traits with POST_INTERACTION flag enabled
    const HAS_EXTENSION = 1n << 2n;       // Bit 2: Has extension data
    const POST_INTERACTION = 1n << 7n;    // Bit 7: Enable post interaction
    const makerTraits = HAS_EXTENSION | POST_INTERACTION;
