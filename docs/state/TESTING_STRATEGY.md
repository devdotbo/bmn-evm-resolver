...
This document outlines the comprehensive testing strategy for migrating the resolver from indexer-dependent to self-managed state. The strategy covers unit tests, integration tests, end-to-end tests, and production validation.

## Testing Principles

1. **Test Independence**: Resolver tests should not depend on indexer availability
...
Deno.test("SecretManager - Cache performance", async () => {
  ...
  assert(cacheTime < dbTime / 10, `Cache (${cacheTime}ms) not faster than DB (${dbTime}ms)`);
});
...
Deno.test("Performance - Memory usage under load", async () => {
  ...
  const storeIncrease = (afterStoreMemory.heapUsed - initialMemory.heapUsed) / 1024 / 1024;
  const cacheIncrease = (afterCacheMemory.heapUsed - afterStoreMemory.heapUsed) / 1024 / 1024;
  ...
  assert(cacheIncrease < 100, `Cache using too much memory: ${cacheIncrease}MB`);
});
```
...
Deno.test("Migration - Dual write to both stores", async () => {
  ...
  const inResolver = await resolver.secretManager.getSecretByHashlock(secret.hashlock);
...
async function validateShadowMode() {
  ...
  for (const check of checks) {
    if (check.value > check.threshold) {
      console.error(`❌ ${check.name} failed: ${check.value} > ${check.threshold}`);
      return false;
    }
  }
...
Following this strategy provides confidence that the resolver's state management is robust, performant, and maintainable.