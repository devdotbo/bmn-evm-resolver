...
const RESOLVER_ADDRESS = "0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC" as Address;
...
console.log(`   HAS_EXTENSION flag (bit 249): ${(traits & (1n << 249n)) !== 0n ? "✅" : "❌"}`);
console.log(`   POST_INTERACTION flag (bit 251): ${(traits & (1n << 251n)) !== 0n ? "✅" : "❌"}`);
...
const postInteractionData = encodePostInteractionData(FACTORY_ADDRESS, escrowParams);
console.log(`   PostInteraction data length: ${(postInteractionData.length - 2) / 2} bytes`);
console.log(`   First 20 bytes (factory): ${postInteractionData.slice(0, 42)}`);
console.log(`   Data starts with factory address: ${postInteractionData.slice(0, 42).toLowerCase() === FACTORY_ADDRESS.toLowerCase() ? "✅" : "❌"}`);
...
const postInteractionOffset = (offsets[28] << 24) | (offsets[29] << 16) | (offsets[30] << 8) | offsets[31];
console.log(`   PostInteraction offset (bytes 28-31): ${postInteractionOffset}`);
console.log(`   Expected offset: ${(postInteractionData.length - 2) / 2}`);
console.log(`   Offset correct: ${postInteractionOffset === (postInteractionData.length - 2) / 2 ? "✅" : "❌"}`);
...
console.log(`   Salt includes extension hash: ${(salt & ((1n << 160n) - 1n)) === extensionHashLast160 ? "✅" : "❌"}`);
...
console.log("\n⚠️  Note: The resolver at", RESOLVER_ADDRESS, "has been whitelisted on the factory.");
console.log("   The 1inch protocol should now trigger postInteraction callbacks after order fills.");