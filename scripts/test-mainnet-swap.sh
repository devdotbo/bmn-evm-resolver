...
# End-to-End Mainnet Swap Test
# Tests BMN swap from Base to Etherlink

set -e

echo "=== Bridge-Me-Not Mainnet E2E Test ==="
echo ""
...
if [ ! -f ".env.mainnet" ]; then
    ...
    echo "Please create .env.mainnet with your private keys"
    exit 1
fi
...
# Main test flow
echo -e "${GREEN}Starting mainnet swap test...${NC}"
...
./run-mainnet-resolver.sh > resolver.log 2>&1 &
RESOLVER_PID=$!
...
if ! ps -p $RESOLVER_PID > /dev/null; then
    echo -e "${RED}Resolver failed to start! Check resolver.log${NC}"
    cat resolver.log
    exit 1
fi
...
echo "Alice will create an order to swap 10 BMN from Base to Etherlink"
wait_for_confirmation

deno run --allow-net --allow-read --allow-write --allow-env --env-file=.env src/alice/create-mainnet-order.ts
...
timeout 30 tail -f resolver.log || true
...
echo "Alice will now withdraw BMN from Etherlink escrow"
wait_for_confirmation
...
echo "Expected result: Alice should have less BMN on Base and more on Etherlink"
echo "                 Bob should have more BMN on Base and less on Etherlink"