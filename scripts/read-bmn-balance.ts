...
async function main() {
  ...
  const client = createPublicClient({ chain: base, transport: http(`https://rpc.ankr.com/base/${ankrKey}`) });
  const BMN = CREATE3_ADDRESSES.BMN_TOKEN as `0x${string}`;
  const balance = await client.readContract({ address: BMN, abi: IERC20Abi.abi, functionName: "balanceOf", args: [address as `0x${string}`] });
  console.log(`BMN balance of ${address} on Base: ${balance} wei (${Number(balance) / 1e18} BMN)`);
}
