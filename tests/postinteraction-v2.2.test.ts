...
function createTestEscrowParams(
  overrides?: Partial<EscrowParams>,
): EscrowParams {
  return {
...
Deno.test("PostInteraction v2.2.0 Unit Tests", async (t) => {
  await t.step("should encode PostInteraction data correctly", () => {
    ...
    assertEquals(
      encoded.slice(0, 42).toLowerCase(),
    ...
    assertEquals(
      encoded.length >= 938,
      true,
      "Encoded data should be at least 938 characters",
    );
  });
  ...
  await t.step("should pack timelocks correctly", () => {
    ...
    assertEquals(
      srcCancellation > now,
      true,
      "Source cancellation should be in the future",
    );
    assertEquals(
      srcCancellation < now + 3700n,
      true,
      "Source cancellation should be within 1 hour",
    );
    ...
    assertEquals(
      dstWithdrawal > now,
      true,
      "Destination withdrawal should be in the future",
    );
    assertEquals(
      dstWithdrawal < now + 400n,
      true,
      "Destination withdrawal should be within 5 minutes",
    );
  });
  ...
  await t.step("should generate maker traits for PostInteraction", () => {
    ...
    assertEquals(
      (traits & (1n << 249n)) > 0n,
      true,
      "Should have HAS_EXTENSION flag at bit 249",
    );
    ...
    assertEquals(
      (traits & (1n << 251n)) > 0n,
      true,
      "Should have POST_INTERACTION flag at bit 251",
    );

    // Combined value should be correct
    assertEquals(
      traits,
      MAKER_TRAITS.HAS_EXTENSION | MAKER_TRAITS.POST_INTERACTION,
    );
  });
  ...
  await t.step("should handle edge case timelocks", () => {
    ...
    assert(
      srcCancel >= now - 1n && srcCancel <= now + 1n,
      "Zero timelock src should be now",
    );
    assert(
      dstWithdraw >= now - 1n && dstWithdraw <= now + 1n,
      "Zero timelock dst should be now",
    );
  });
  ...
  await t.step("should handle edge case deposits", () => {
    ...
    assertEquals(
      extractedSrc,
      maxUint128,
      "Max src deposit should unpack correctly",
    );
    assertEquals(
      extractedDst,
      maxUint128,
      "Max dst deposit should unpack correctly",
    );

    // Asymmetric deposits
    const asymmetric = packDeposits(
      parseUnits("1", 18),
      parseUnits("1000", 18),
    );
...
Deno.test("PostInteraction v2.2.0 Integration Tests", async (t) => {
  await t.step("should encode complete escrow creation data", () => {
    const params = createTestEscrowParams();
    const encoded = encodePostInteractionData(
      TEST_ADDRESSES.FACTORY_V2_2,
      params,
    );
    ...
    assert(
      encoded.length >= expectedMinLength,
      `Encoded length ${encoded.length} should be at least ${expectedMinLength}`,
    );
  });
  ...
  await t.step("should handle cross-chain escrow scenarios", () => {
    ...
    const sameChainEncoded = encodePostInteractionData(
      TEST_ADDRESSES.FACTORY_V2_2,
      sameChainParams,
    );
    ...
    const crossChainEncoded = encodePostInteractionData(
      TEST_ADDRESSES.FACTORY_V2_2,
      crossChainParams,
    );
    ...
    const highValueEncoded = encodePostInteractionData(
      TEST_ADDRESSES.FACTORY_V2_2,
      highValueParams,
    );
  ...
  await t.step("should encode different timelock configurations", () => {
    ...
    const fastEncoded = encodePostInteractionData(
      TEST_ADDRESSES.FACTORY_V2_2,
      fastParams,
    );
    ...
    const standardEncoded = encodePostInteractionData(
      TEST_ADDRESSES.FACTORY_V2_2,
      standardParams,
    );
    ...
    const longTermEncoded = encodePostInteractionData(
      TEST_ADDRESSES.FACTORY_V2_2,
      longTermParams,
    );
  ...
  await t.step("should generate unique escrow addresses", () => {
    ...
    const encoded1 = encodePostInteractionData(
      TEST_ADDRESSES.FACTORY_V2_2,
      params1,
    );
    const encoded2 = encodePostInteractionData(
      TEST_ADDRESSES.FACTORY_V2_2,
      params2,
    );
    const encoded3 = encodePostInteractionData(
      TEST_ADDRESSES.FACTORY_V2_2,
      params3,
    );
  ...
  await t.step(
    "should handle maker traits correctly for different order types",
    () => {
      // PostInteraction order
      const postInteractionTraits = MAKER_TRAITS.forPostInteraction();
      assert(
        (postInteractionTraits & MAKER_TRAITS.HAS_EXTENSION) > 0n,
        "Should have extension flag",
      );
      assert(
        (postInteractionTraits & MAKER_TRAITS.POST_INTERACTION) > 0n,
        "Should have post interaction flag",
      );

      // Regular order (no post interaction)
      const regularTraits = 0n;
      assert(
        (regularTraits & MAKER_TRAITS.POST_INTERACTION) === 0n,
        "Regular order should not have post interaction",
      );

      // Extension-only order (no post interaction)
      const extensionOnlyTraits = MAKER_TRAITS.HAS_EXTENSION;
      assert(
        (extensionOnlyTraits & MAKER_TRAITS.HAS_EXTENSION) > 0n,
        "Should have extension flag",
      );
      assert(
        (extensionOnlyTraits & MAKER_TRAITS.POST_INTERACTION) === 0n,
        "Should not have post interaction flag",
      );
    },
  );
});
...
Deno.test("PostInteraction v2.2.0 Error Scenarios", async (t) => {
  ...
  await t.step("should handle invalid amounts", () => {
    ...
    const maxEncoded = encodePostInteractionData(
      TEST_ADDRESSES.FACTORY_V2_2,
      maxAmountParams,
    );
  ...
  await t.step("should handle invalid timelocks", () => {
    ...
    assertExists(
      encoded,
      "Past timelocks should encode (validation happens on-chain)",
    );
  });
  ...
  await t.step("should handle invalid hashlocks", () => {
    ...
    const zeroEncoded = encodePostInteractionData(
      TEST_ADDRESSES.FACTORY_V2_2,
      zeroHashlockParams,
    );
  ...
  await t.step("should handle mismatched token decimals", () => {
    ...
    const usdcEncoded = encodePostInteractionData(
      TEST_ADDRESSES.FACTORY_V2_2,
      usdcParams,
    );
    ...
    const nftEncoded = encodePostInteractionData(
      TEST_ADDRESSES.FACTORY_V2_2,
      nftParams,
    );
...
Deno.test("PostInteraction v2.2.0 Performance Benchmarks", async (t) => {
  await t.step("should encode data efficiently", () => {
    ...
    assert(
      avgTime < 1,
      `Encoding should be fast (avg: ${avgTime.toFixed(3)}ms)`,
    );
  });
  ...
  await t.step("should generate nonces efficiently", () => {
    ...
    assert(
      avgTime < 0.1,
      `Nonce generation should be very fast (avg: ${avgTime.toFixed(4)}ms)`,
    );
  });
  ...
  await t.step("should pack timelocks efficiently", () => {
    ...
    assert(
      avgTime < 0.1,
      `Timelock packing should be very fast (avg: ${avgTime.toFixed(4)}ms)`,
    );
  });
  ...
  await t.step("should handle batch encoding efficiently", () => {
    ...
    for (let batch = 0; batch < batches; batch++) {
      ...
      for (let i = 0; i < batchSize; i++) {
        const params = createTestEscrowParams({
          nonce: BigInt(batch * batchSize + i),
        });
        const encoded = encodePostInteractionData(
          TEST_ADDRESSES.FACTORY_V2_2,
          params,
    ...
    console.log(
      `  ⚡ Throughput: ${
        (totalOperations / (totalTime / 1000)).toFixed(0)
      } ops/sec`,
    );
    ...
    assert(
      avgTime < 2,
      `Batch encoding should be efficient (avg: ${avgTime.toFixed(3)}ms)`,
    );
  });
});
...
Deno.test("PostInteraction v2.2.0 Advanced Integration", async (t) => {
  ...
  await t.step("should support different token pairs", () => {
    ...
    const sameTokenEncoded = encodePostInteractionData(
      TEST_ADDRESSES.FACTORY_V2_2,
      sameTokenParams,
    );
    ...
    const differentTokenEncoded = encodePostInteractionData(
      TEST_ADDRESSES.FACTORY_V2_2,
  ...
  await t.step("should calculate escrow address deterministically", () => {
    ...
    assertEquals(
      encoded1,
      encoded2,
      "Same parameters should produce same encoding",
    );
    ...
    assertNotEquals(
      encoded1,
      encoded3,
      "Different nonce should produce different encoding",
    );
  });
  ...
  await t.step("should support atomic swap workflows", () => {
    ...
    const srcDepositRatio =
      Number(formatUnits(atomicSwapParams.srcSafetyDeposit, 18)) /
      Number(formatUnits(atomicSwapParams.srcAmount, 18));
    const dstDepositRatio =
      Number(formatUnits(atomicSwapParams.dstSafetyDeposit, 18)) /
      Number(formatUnits(atomicSwapParams.dstAmount, 18));

    assert(
      Math.abs(srcDepositRatio - 0.1) < 0.01,
      "Source deposit should be ~10%",
    );
    assert(
      Math.abs(dstDepositRatio - 0.1) < 0.01,
      "Destination deposit should be ~10%",
    );
  });
});
...
Deno.test("PostInteraction v2.2.0 Mock Contract Tests", async (t) => {
  await t.step("should simulate successful escrow creation", async () => {
    ...
    const mockPostInteraction = (
      data: string,
    ): { success: boolean; escrowAddress?: string; error?: string } => {
    ...
    assertEquals(
      result.escrowAddress?.length,
      42,
      "Escrow address should be valid format",
    );
  });

  await t.step("should simulate escrow creation failures", async () => {
    // Test various failure scenarios
    const mockPostInteractionWithErrors = (
      data: string,
      scenario: string,
  ...
  await t.step("should validate extension data before submission", () => {
    ...
    const validateExtensionData = (
      ...
      if (data.length < 938) {
        errors.push(
          `Data too short: ${data.length} chars, minimum 938 required`,
        );
      }
    ...
    const validData = encodePostInteractionData(
      TEST_ADDRESSES.FACTORY_V2_2,
      validParams,
    );
...
Deno.test("PostInteraction v2.2.0 1inch Extension Format", async (t) => {
  await t.step(
    "should encode 1inch extension with correct offset structure",
    () => {
      const params = createTestEscrowParams();
      ...
      assertEquals(
        dataLength,
        actualDataLength,
        "Offset should contain correct data length",
      );
      ...
      assertEquals(
        extractedData,
        postInteractionData,
        "PostInteraction data should match",
      );
    },
  );
  ...
  await t.step("should include extension hash in salt (lower 160 bits)", () => {
    ...
    assertExists(
      lower160Bits,
      "Should extract lower 160 bits of extension hash",
    );
    assertEquals(
      lower160Bits.length,
      42,
      "Lower 160 bits should be 20 bytes (40 hex + 0x)",
    );
    ...
    assertEquals(
      fullSalt.length,
      66,
      "Full salt should be 32 bytes (64 hex + 0x)",
    );
    assert(
      fullSalt.endsWith(lower160Bits.slice(2)),
      "Salt should end with extension hash",
    );
  });

  await t.step(
    "should properly encode PostInteraction data for factory",
    () => {
      const params = createTestEscrowParams({
        srcImplementation: TEST_ADDRESSES.SRC_IMPL,
        dstImplementation: TEST_ADDRESSES.DST_IMPL,
      ...
      assertEquals(
        ...
        expectedLength,
        "Encoded params should have correct length",
      );
  ...
  await t.step("should validate complete 1inch extension workflow", () => {
    ...
    assert(
      (makerTraits & (1n << 249n)) > 0n,
      "Should have HAS_EXTENSION at bit 249",
    );

    // Validate bit 251 (POST_INTERACTION)
    assert(
      (makerTraits & (1n << 251n)) > 0n,
      "Should have POST_INTERACTION at bit 251",
    );
    ...
    assert(
      order.salt.endsWith(extensionHashLower160.slice(2)),
      "Salt should end with extension hash",
    );
    ...
    assertEquals(
      extractedPostInteraction,
      postInteractionData,
      "Should extract correct PostInteraction data",
    );
  });
  ...
  await t.step("should handle edge cases in 1inch extension format", () => {
    ...
    const minPostInteraction = encodePostInteractionData(
      TEST_ADDRESSES.FACTORY_V2_2,
      minParams,
    );
    ...
    assertEquals(
      minDataLength,
      (minExtension.length - 66) / 2,
      "Offset should match actual data length",
    );
    ...
    const maxPostInteraction = encodePostInteractionData(
      TEST_ADDRESSES.FACTORY_V2_2,
      maxParams,
    );
    ...
    assertEquals(
      maxDataLength,
      (maxExtension.length - 66) / 2,
      "Max params offset should match data length",
    );
  });

  await t.step(
    "should correctly compare with incorrect bit flag implementations",
    () => {
      ...
      assertNotEquals(
        wrongTraits,
        correctTraits,
        "Wrong and correct traits should differ",
      );
      ...
      assert(
        correctTraits > (1n << 248n),
        "Correct traits should be huge number",
      );
      ...
      assertEquals(
        implementedTraits,
        correctTraits,
        "Implementation should use correct bit flags",
      );
    },
  );
});
...
Deno.test("PostInteraction v2.2.0 Limit Order Integration", async (t) => {
  ...
  await t.step("should handle order fill with escrow creation", () => {
    ...
    const mockOrderFill = (orderId: string, extensionData: string): {
      ...
      const mockEscrowAddress = `0x${
        keccak256(extensionData as Hex).slice(26)
      }`;
      const mockTxHash = `0x${
        keccak256(encodePacked(["bytes32"], [orderId as Hex])).slice(2)
      }`;
...
if (import.meta.main) {
  ...
  console.log(
    "  ✅ 1inch Extension Format Tests - Bit flags 249/251, offset structure, extension hash",
  );
